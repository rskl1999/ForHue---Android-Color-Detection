...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    //new fit to screen camera
    private final Matrix mMatrix = new Matrix();

    private void updateMatrix() {
        float mw = this.getWidth(); //matrix
        ...
        boolean isFrontCamera = mCameraIndex == CAMERA_ID_FRONT; //mCameraIndex is our active camera

        mMatrix.reset();
        if (isFrontCamera) {
            mMatrix.preScale(-1, 1, hw, hh); //MH - this will mirror the camera
        }
        mMatrix.preTranslate(hw, hh); //rotates cam, move to center

        if (isFrontCamera){  //rotate differently and centered ---------------------
            mMatrix.preRotate(270);
        } else {
            mMatrix.preRotate(90);
        }
        mMatrix.preTranslate(-hw, -hh);
        mMatrix.preScale(scale,scale,hw,hh); //scale it to fill the width of the screen
    }
...
/**
           //1st change
            ...
            float mScale1=0;
                ...
                if(canvas.getHeight()>canvas.getWidth()){
                   ...
                   mScale2=(float)canvas.getWidth()/(float)mCacheBitmap.getHeight(); //1.8 m1
                   mScale1=(float)canvas.getHeight()/(float)mCacheBitmap.getWidth();//1.5
        ...
        if (mScale != 0) {
                ...
                //added-----------------------
        /**    if (mScale1 != 0) { //will fit canvas to phone
                    canvas.drawBitmap(mCacheBitmap, new Rect(0,0,mCacheBitmap.getWidth(), mCacheBitmap.getHeight()),
